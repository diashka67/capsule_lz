...
from log import log #импортируем декоратор


class AAPL:
    def __init__(self, file_path, start_date='2016-12-12', end_date='2022-01-09'):
        self.df = pd.read_csv(file_path)
        self.df['Date'] = pd.to_datetime(self.df['Date'])   # переводим строки с датами в datetime
        self.df = self.df[(self.df['Date'] >= start_date) & (self.df['Date'] <= end_date)]   # берем только нужный промежуток времени
        self.df = self.df.reset_index(drop=True)   #нумеруем строки по порядку
    ...
    def plot_stock_prices(self):
        ...
        # делим дни на те, где цена выросла и где упала
        ...
        # Тени, показвают максимальную и минимальную цену
        ...
        # отступы по краям графика, чтобы свечки по краям не обрезались
        max_index = len(self.df) - 1
        plt.xlim(-0.5, max_index + 0.5)

        # показываем даты на оси X (5 штук, чтобы подписи не налезали друг на друга)
        step = max(1, len(self.df) // 5)
        plt.xticks(
                ticks = range(0, len(self.df), step), 
                labels = self.df['Date'].iloc[::step].dt.strftime('%d.%m.%Y'),
                rotation=45, # поворачиваем подписи, чтоб было читаемо
                color='white'
        )
